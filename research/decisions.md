...
- 日期（Date）：2026-06-20
- 决策（Decision）：采用单分支、无人审核的自动进化循环；允许外部变更命令修改任意仓库文件，但仅在测试、`dev` 提升和 `regression`/`heldout` 非退化后自动提交。
- 背景（Context）：用户明确要求自动进化、可修改任何内容，并选择单分支连续自改。
- 推理（Reasoning）：以提交作为轮次事务边界可在不引入人工审核或 worktree 的前提下自动保留获胜候选并恢复失败候选。
- 后果（Consequences）：自动循环运行期间必须独占工作树；候选评估失败、测试失败或未达门槛都会回滚。
