...
- 2026-06-21 的 live `dev` 评估在 AGENT1 结束后无法继续：本地 `doctor` 返回 HTTP 502，`live-20260621-dev/` 只留下完整的 AGENT1 和不完整的 AGENT2 日志。因此在 BalatroBot 恢复且从新局独占执行前，不能将该目录用于策略胜率、跨 seed 对比或候选晋升。详见 `research/runs/2026-06-21.md`。
- 2026-06-20 当前默认策略的评估失败通常不是弃牌耗尽，而是出牌耗尽：33 个有终局记录的 eval 失败中 29 个以 3--4 次弃牌、0 次出牌结束。下一轮手牌策略应将高门槛盲注下的弃牌视为主动重抽/牌型构筑资源，而不能只在局部弱牌时提供低优先级弃牌候选；该结论尚未在更多 seed 或人工对局中验证。
- 通关的核心方法论应为"渐进式牌型专精"：开局 Ante 1–2 不固定牌型，灵活探索；Ante 3–5 根据 Joker 信号锁定目标牌型；Ante 6–8 集中所有资源（Joker、塔罗、星球、弃牌雕塑）打造该牌型。这一策略需要显式的 commitment 状态机贯穿所有 Agent 决策。详见 [[progressive-hand-commitment]]。
